...
Box2DDrawer::Box2DDrawer(SDL_Renderer *r, float s = 1.f) : renderer(r), scale(s) {}

void Box2DDrawer::SetScale(float s) {
    scale = s;
}
...
void Box2DDrawer::DrawCircle(const b2Vec2 &center, float32 radius, const b2Color &color) {
    ...
    for (int32 i = 0; i < k_segments; ++i)
    {
        ...
        SDL_RenderDrawLine(renderer, (int) (v1.x * scale), (int) (v1.y * scale), (int) (v2.x * scale), (int) (v2.y * scale));
        r1 = r2;
        v1 = v2;
    }
}
...
void Box2DDrawer::DrawPoint(const b2Vec2 &p, float32 size, const b2Color &color) {
    ...
    const SDL_Rect point = {
            (int) (p.x * scale - halfSize),
            (int) (p.y * scale - halfSize),
            halfSize * 2,
            halfSize * 2
    };
